  ...
  sequenceA $ Map.foldlWithKey
    (\accum sWitInd eExecUnits -> do
      case List.lookup sWitInd scriptMapping of
        ...
        -- TODO: We can potentially render the txinput (in place of the script hash) at which
        -- the reference script lives if we include the tx body in this function's parameters
        -- and using the ScriptWitnessIndex
